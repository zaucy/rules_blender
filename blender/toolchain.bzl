BlenderInfo = provider(
    doc = "Information about how to invoke the tool executable.",
    fields = {
        "target_tool_path": "Path to the blender executable for the target platform.",
        "tool_files": """Files required in runfiles to make the blender executable available.

May be empty if the target_tool_path points to a locally installed blender binary.""",
    },
)
...
def _blender_toolchain_impl(ctx):
    ...
    # Make the $(BLENDER_BIN) variable available in places like genrules.
    # See https://docs.bazel.build/versions/main/be/make-variables.html#custom_variables
    template_variables = platform_common.TemplateVariableInfo({
        "BLENDER_BIN": target_tool_path,
    })
    default = DefaultInfo(
